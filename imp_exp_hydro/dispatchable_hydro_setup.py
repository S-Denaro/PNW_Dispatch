...
np.savetxt('PGE_V_minflow.txt',PGE_V_min_filtered)
np.savetxt('SCE_minflow.txt',SCE_min_filtered)

#ramp rates
PGE_V_down = np.percentile(downramps[:,0,2:],90)
PGE_V_up = np.percentile(upramps[:,0,2:],90)
SCE_down = np.percentile(downramps[:,1,2:],90)
SCE_up = np.percentile(upramps[:,1,2:],90)
    
#max capacity
PGE_V_cap = np.max(PGE_V_max)
SCE_cap = np.max(SCE_max)
