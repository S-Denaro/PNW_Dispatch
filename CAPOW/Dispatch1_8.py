...
model.Generators = model.Zone1Generators | model.Zone2Generators | model.Zone3Generators | model.Zone4Generators | model.WECCImports
...
def SysCost(model):
    ...
    fixed_gas1 = sum(model.no_load[j]*model.on[j,i]*model.GasPrice['PGE_valley'] for i in model.hh_periods for j in model.Zone1Gas)
    fixed_gas2 = sum(model.no_load[j]*model.on[j,i]*model.GasPrice['PGE_bay'] for i in model.hh_periods for j in model.Zone2Gas)
    fixed_gas3 = sum(model.no_load[j]*model.on[j,i]*model.GasPrice['SCE'] for i in model.hh_periods for j in model.Zone3Gas)
    fixed_gas4 = sum(model.no_load[j]*model.on[j,i]*model.GasPrice['SDGE'] for i in model.hh_periods for j in model.Zone4Gas)
    ...
    return fixed_coal + fixed_gas1 + fixed_gas2 + fixed_gas3 + fixed_gas4 + fixed_oil + fixed_slack + coal1 + coal2 + coal3 + gas1_1 + gas1_2 + gas1_3 + gas1_4 + gas2_1 + gas2_2 + gas2_3 + gas2_4 + gas3_1 + gas3_2 + gas3_3 + gas3_4 + oil1 + oil2 + oil3 + psh1 + psh2 + psh3 + slack1 + slack2 + slack3 + starts + exchange
...
def Zone3_Balance(model,i):
    ...
    imports = sum(model.flow[s,'SCE',i] for s in model.sources) + model.P46I_SCE_minflow[i] + model.mwh_1['P46I_SCE',i] + model.mwh_2['P46I_SCE',i] + model.mwh_3['P46I_SCE',i] + model.P61I_minflow[i] + model.mwh_1['P61I',i] + model.mwh_2['P61I',i] + model.mwh_3['P61I',i] + model.mwh_1['P42I',i] + model.mwh_2['P42I',i] + model.mwh_3['P42I',i]
